# Progress Reports

Progress reports are the regular heartbeat of a funded program. They serve three audiences simultaneously: the program manager assessing your progress, the finance team tracking spend, and your future self preparing the final report or next proposal.

## Structure

### 1. Header
- Program name, contract/grant number
- Reporting period
- Team lead / PI
- Report number (e.g., "Quarterly Report Q2, Months 4-6")

### 2. Executive Summary (half page)
- Top 3 accomplishments this period
- Top risk or issue
- Headline metric progress
- Any deviation from plan

### 3. Technical Progress by Task

For each SOW task:

**Task X.Y: [Title]**
- **Status:** On Track / At Risk / Delayed / Complete
- **Progress this period:** What was done, with key results (quantitative)
- **Planned vs. actual:** How does progress compare to the schedule?
- **Key results/data:** Tables, figures, or brief data summaries
- **Issues encountered:** What went wrong, what was learned
- **Next period plan:** What will be done in the coming reporting period

Use a traffic-light system for at-a-glance status:
- Green: On track, no issues
- Yellow: Minor delays or issues, mitigation in progress
- Red: Significant risk to milestone, intervention needed

### 4. Milestone Status

| Milestone | Target Date | Status | Current Progress | Notes |
|-----------|-----------|--------|-----------------|-------|
| M1: ≥50% delignification | Month 8 | Green | 47% achieved on birch (Month 5) | On track |
| M2: ≥50% monomer yield | Month 10 | Yellow | 38% on hardwood, 18% softwood | Softwood optimization ongoing |

### 5. Risk Register Update

| Risk | Status | Likelihood | Impact | Mitigation | Update |
|------|--------|-----------|--------|-----------|--------|
| Softwood recalcitrance | Active | High | Medium | Alternative catalyst screening | Added Cu/Ni dual-site catalyst to screening panel |

### 6. Financial Summary
- Budget vs. actual spend by category
- Burn rate and projected remaining budget
- Any reallocation requests

### 7. Publications, Presentations, IP
- Papers submitted/published
- Conference presentations
- Invention disclosures filed
- Any IP concerns

### 8. Plans for Next Period
- Key activities by task
- Upcoming milestones or decision points
- Resource needs or schedule changes

## Writing Principles

- **Lead with results, not activities.** "Achieved 47% delignification on birch" not "Performed RCF experiments on birch."
- **Be honest about problems.** Program managers respect teams that identify issues early and propose solutions. Hiding problems until they're crises destroys trust.
- **Quantify everything.** Numbers, not adjectives. "Significant progress" is meaningless; "increased monomer yield from 31% to 47%" is informative.
- **Connect results to milestones.** Every result should map to a program metric. If you're doing work that doesn't map to a milestone, either it needs to or you're off-scope.
- **Keep it concise.** Program managers read dozens of these. 5-10 pages for a quarterly report is typical. Dense content beats padded pages.
- **Use consistent formatting across reports.** The reader should be able to flip to "Task 2.1" in every quarterly report and see how it evolved over time.
